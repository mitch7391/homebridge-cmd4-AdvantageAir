...
name=""
fanSetup="fan"
AdvAir_shPath="/usr/local/lib/node_modules/homebridge-cmd4-advantageair/AdvAir.sh"
...
echo "Do you want to set up your \"Fan\" as \"FanSwitch\"? (y/n)" 
read -r INPUT
if [[ "${INPUT}" = "y" || "${INPUT}" = "Y" ]]; then
   fanSetup="fanSwitch"
fi  
 
if [ ! -f "${AdvAir_shPath}" ]; then
   AdvAir_shPath=""
   until [ -n "${AdvAir_shPath}" ]; do
      echo ""
...
function readHomebridgeConfigJson()
{
   ...
   if [ ! -f "${homebridgeConfigJson}" ]; then
      ...
      until [ -n "${INPUT}" ]; do 
         ...
         echo "otherwise just hit enter to abort copying."
   ...
   validFile=$(grep -n '"platforms"' "${homebridgeConfigJson}"|cut -d":" -f1)
   if [ -n "${validFile}" ]; then
      if [ -z "${INPUT}" ]; then
         echo ""
         echo "Found the Homebridge config.json: ${homebridgeConfigJson}"
      fi
      ...
      echo ""
      echo "ERROR: no valid Homebridge config.json found or specified!"
...
for ((n=1; n<=noOfTablets; n++)); do
   ...
   if [ -z "${sysType}" ]; then
      echo ""
      echo "ERROR: jq failed.  Please make sure jq is installed."
      exit 1
   fi
   ...
   if [ "$hasAircons" ]; then
      for (( a=1;a<=4;a++ )); do
         ...
         if [ "${aircon}" != "null" ]; then
            ...
            if [ "${fanSetup}" = "fanSwitch" ]; then
               cmd4FanSwitch "${cmd4ConfigAccessoriesAA}" "${name} Fan"
               cmd4FanLinkTypes "${cmd4ConfigAccessoriesAA}" "${name} FanSpeed"
            else
...
if [[ "${INPUT}" = "y" || "${INPUT}" = "Y" ]]; then
   ...
   if [ -n "${configJson}" ]; then
      ...
      echo ""
      echo "Reatart Homebridge for the config to take effect, OR"
      echo ""
      echo "if you know what you are doing you can do some edits within JASON Config Editor, like changing some names"
      echo "or deleting some accessories you do not need, etc," 
      echo "click SAVE when you are done, then restart Homebridge"
   else
      echo ""
      ...
      echo "Please copy the content of the file \"${cmd4ConfigJsonAA}\" to Homebridge Cmd4 JASON Config Editor"
      echo ""
      echo "SAVE it then reatart Homebridge, OR"
      echo ""
      echo "if you know what you are doing you can do some edits within JASON Config Editor, like changing some names"
      echo "or deleting some accessories you do not need, etc," 
      echo "click SAVE when you are done, then restart Homebridge"
