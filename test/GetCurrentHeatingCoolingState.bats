...
@test "AdvAir ( ezone        ) Test PassOn3 Get CurrentHeatingCoolingState" {
   # We symbolically link the directory of the test we want to use.
   ln -s ./testData/dataPassOn3 ./data
   _common_compareAgainstEzone Get Blah CurrentHeatingCoolingState TEST_ON
}

@test "AdvAir ( ezone        ) Test PassOn5 Get CurrentHeatingCoolingState" {
   # We symbolically link the directory of the test we want to use.
   ln -s ./testData/dataPassOn5 ./data
   _common_compareAgainstEzone Get Blah CurrentHeatingCoolingState TEST_ON
}

@test "AdvAir ( ezone        ) Test FailOn5 Get CurrentHeatingCoolingState" {
