...
test('restored thermostats, switches and sensors are not logged as newly created accessories', async t => {
  ...
  for (const mode of ['vent', 'dry']) {
    const accessory = new c.api.platformAccessory(mode,
      c.api.hap.uuid.generate(JSON.stringify([thermostatIdentity, 'mode-fan', mode])));
...
test('platform discovers both a zone switch and its separate temperature sensor', async (t) => {
  ...
  c.api.emit('didFinishLaunching');
  await flushPromises();
  assert.equal(c.allRegistrations.length, 5);
...
test('platform restores a cached switch without exposing its saved state before discovery', async (t) => {
  ...
  assert.equal(c.allRegistrations.length, 4);
  assert.equal(c.registrations.length, 1);
});
