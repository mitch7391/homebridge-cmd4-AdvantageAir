...
@test "AdvAir ( ezone inline ) Test PassOn1 Set TargetHeatingCoolingState 1" {
   ...
   e_status=$status
   e_lines=("${lines[@]}")
   run ./compare/AdvAir.sh Set Blah TargetHeatingCoolingState 1 TEST_ON
   assert_equal "$status" "$e_status" ]
   assert_equal "${lines[0]}" "${e_lines[0]}"
}

@test "AdvAir ( ezone        ) Test PassOn1 Set TargetHeatingCoolingState 1" {
   ln -s ./testData/dataPassOn1 ./data
   _common_compareAgainstEzone Set Blah TargetHeatingCoolingState 1 TEST_ON
}
