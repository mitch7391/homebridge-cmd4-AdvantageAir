...
@test "AdvAir Test Set TargetDoorState thing:Garage" {
   beforeEach
   # Issue the reInit
...
@test "AdvAir Test Set TargetDoorState thiID:6801801" {
   beforeEach
   # Issue the reInit
   curl -s -g "http://localhost:$PORT/reInit"
   # Do the load
   curl -s -g "http://localhost:$PORT?load=testData/myPlace.txt"
   run ../AdvAir.sh Set "Garage" TargetDoorState 0 thiID:6801801 127.0.0.1 TEST_ON
   assert_equal "$status" 0
   assert_equal "${lines[0]}" "Try 0"
   assert_equal "${lines[1]}" "Parsing for jqPath: .aircons.ac1.info"
...
@test "AdvAir Test Set TargetDoorState thiID:6801801 flip enabled" {
   beforeEach
   # Issue the reInit
   curl -s -g "http://localhost:$PORT/reInit"
   # Do the load
   curl -s -g "http://localhost:$PORT?load=testData/myPlace.txt"
   run ../AdvAir.sh Set "Garage" TargetDoorState 0 thiID:6801801 127.0.0.1 flip TEST_ON
   assert_equal "$status" 0
   assert_equal "${lines[0]}" "Try 0"
   assert_equal "${lines[1]}" "Parsing for jqPath: .aircons.ac1.info"
